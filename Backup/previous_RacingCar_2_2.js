...
function coinGather(moveableObject, stayedObject, stayedObjectTorso) { // here will be added textblock later
    ...
    if (coinFlag == 1) {
        if (coinCounter < 32) {
            ...
            stayedObjectTorso.translate(new BABYLON.Vector3(1, 1, -2), 0.05, BABYLON.Space.LOCAL);
            coinCounter += 1;
        }
        else {
            coinFlag = 0;
    ...
    var createScene = async function () {
        ...
        for (let icoin = 0; icoin < numCoins; icoin++) {
            ...
            meshTaskCoin.onSuccess = function (task) {
                ...
                var coinPlane = BABYLON.MeshBuilder.CreateGround("ground", { width: 0.25, height: 0.25 }, scene);
        ...
        assetsManagerCity.onFinish = function (task) {
            ...
            scene.registerAfterRender(function () {
                ...
                for (let icoin = 0; icoin < numCoins; icoin++) {
                    coinGather(carFrontier, coinsPlanes[icoin], coinsTorsos[icoin]);
